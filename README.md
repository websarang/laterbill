...
청구서에서는 추상적인 `첫 행동·완료 조건` 대신 **지금 바로 할 일·여기까지 되면 끝**으로
표시합니다. 마지막 대화에 맞춰 “최근 발송 1건 확인”, “첫 1분 SRT 적용”, “첫 빌드 오류와
관련 파일 기록”처럼 대상과 결과물이 보이는 문장을 만듭니다.
...
채팅에서는 `/laterbill`을 입력하면 수집 후 텍스트 청구서를 먼저 보여주고, 같은 장부의
비공개 HTML 청구서를 생성해 브라우저에서 엽니다.
먼저 수집 범위만 확인하려면 `/laterbill 수동`을 입력하고, 요약을 확인한 뒤
`청구서 발행` 또는 `발행 취소`를 선택합니다. 발행을 확인하면 텍스트와 HTML이 함께 생성됩니다.
...
python laterbill/scripts/run.py --max-items 10 --also-html bill.html
...
한글 파이프라인, 자동·수동 발행 분리, 텍스트·HTML 이중 출력, HTML 주입 차단과 네트워크 부재를 포함한 **35개 검증**을 실행합니다.
`python tools/verify-release.py`는 Skill ZIP을 새로 만들고 임시 폴더에 압축 해제한 뒤 동일 검증을
다시 수행합니다. [실제 검증 로그](docs/assets/proof/release-verification.txt)에는 양쪽 `35/35`,
