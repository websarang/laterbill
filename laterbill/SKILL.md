---
name: laterbill
description: 「하다 만 일 종결반 (Laterbill)」 — 로컬 AI 대화에 남은 미완료 업무를 추적하고, 세 가지 실행 가능한 종결안을 제시하는 상환 에이전트다. `/laterbill` 호출 시 텍스트 청구서를 먼저 보여주고 같은 장부의 HTML 청구서를 자동 생성해 연다. `/laterbill 수동` 호출 시 수집 요약을 먼저 보여준 뒤 사용자의 `청구서 발행` 확인을 기다리고, 확인 후 텍스트와 HTML을 함께 발행한다. Claude Code·Codex의 프로젝트와 마지막 대화로 돌아가는 좌표를 보존하며 빠른 진전·장애물 해소·완결 우선의 종결안 3개와 자가개선 가드레일을 만든다. 사용자가 "하다 만 일 종결반", "내가 미룬 일", "뭐 하다 말았지", "중단된 프로젝트", "unfinished projects", "what did I abandon"처럼 미완료 업무를 점검할 때도 사용한다. 읽기 전용·로컬 전용이며 민감 원문은 승인 전에 숨긴다.
---
...
- `/laterbill` 또는 별도 모드가 없는 자연어 호출: 비공개 HTML 절대경로를 정한 뒤
  `scripts/run.py --max-items 10 --also-html "<private-bill.html>"`을 실행하라. 반환된
  Markdown 청구서를 채팅에 먼저 표시하고, 생성된 HTML 파일을 브라우저에서 열어 최종
  청구서가 보이게 하라. 텍스트나 HTML 중 하나만 제공하지 마라. 추가 확인을 요구하지 마라.
- `/laterbill 수동`: `scripts/run.py --manual --max-items 10`을 실행해 수집 건수와
  런타임별 통계만 보여주고 멈춰라. 프로젝트명·경로·세션·원문·상환안은 아직 출력하지 마라.
- 수동 모드 뒤 사용자가 `청구서 발행`이라고 하면 `--also-html`을 사용해 Markdown을 먼저
  표시하고 같은 장부의 HTML을 생성·열어라. `발행 취소`라고 하면 아무것도 발행하지 말고
  종료하라.
- 자동·수동은 발행 시점만 다르다. 두 모드 모두 수집은 읽기 전용이고 민감 원문 승인
  경계를 유지한다.

```text
/laterbill          # 자동: 수집 → 텍스트 청구서 → HTML 청구서 열기
/laterbill 수동     # 수동: 안전한 수집 요약 → 확인 → 텍스트 + HTML 발행
```
...
python scripts/run.py --max-items 10 --also-html bill.html
...
`done_when`, `tradeoff`를 포함하라. 화면에는 `first_action`을 **지금 바로 할 일**,
`done_when`을 **여기까지 되면 끝**으로 표시하라. 마지막 대화에서 확인되는 작업 대상,
열 파일·실행할 확인·남길 결과물을 문장에 넣고, “상태 확인”이나 “결과 기록”만 반복하지
마라. 근거가 부족하거나 프로젝트가 없으면 가능한 안만 제시하고 부족한 이유를 표시하라.

예: `sent` 상태 질문에는 최근 발송 1건의 API 응답·발송 기록·수신 여부를 확인하게 하고,
종료 기준은 `sent`가 어느 단계인지 증거로 설명할 수 있는 상태로 쓴다. 자막 작업에는 전체
처리 전에 첫 1분 SRT를 영상에 적용하게 하고, 종료 기준은 순서와 타임코드가 맞는 상태로
쓴다. 사실로 확인되지 않은 파일명·명령·서비스 동작은 만들지 마라.
